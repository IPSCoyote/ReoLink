...
trait reolinkFunctions
{
    ...
    protected function ReolinkGetImage(int $channel = 0)
    {
        // returns Image Array
        // Array
        // (
    ...
    protected function ReolinkSetImage(array $ImageArray, int $channel = 0)
    {
        // Sets Image Array
        // Array
        // (
    ...
    protected function ReolinkSnap(int $channel = 0)
    {
        // returns a Snapshot
        $this->toDebugLog("ReolinkSnap called");

        $ch = curl_init("http://".trim($this->ReadPropertyString("IPAddressDevice"))."/cgi-bin/api.cgi?cmd=Snap&channel=".$channel."&token=" . $this->ReadAttributeString("Token"));
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($ch);
